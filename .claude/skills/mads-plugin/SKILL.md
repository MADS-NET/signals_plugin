...
This skill documents MADS **2.4.3**, plugin protocol
**P8** (hosts accept P7 and newer).
...
mads <source|filter|sink> <name.plugin> [-n agent_name] [-o key=value]
  ...
  ├─ reject the plugin if its protocol is older than P7
...
<!-- Generated by `mads plugin` from MADS 2.4.3. Refresh with `mads plugin --update`. -->
